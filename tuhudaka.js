...
!(async () => {
    if(userCookie) {
        ...
        if(daytype === 0){
            ...
            console.log('【常规脚本】非工作日，检查一次');
            notifyStr += '【常规脚本】非工作日，检查一次'
        }
		await $.wait(1000);
            }
        }
...
async function doSign(sessionid) {
    try {
        ...
        if(!result) return
        if (result.code === 0){
            let msg = `【常规脚本】途虎签到成功，签到时间：${result.data.attendance_time}\n`
            notifyStr += msg
            console.log(msg)
        }else {
            let msg = `【常规脚本】途虎签到失败，错误数据：${JSON.stringify(result)}\n`
            notifyStr += msg
            console.log(msg)
        }
    } catch (e) {
...
async function doReport(sessionid){
      try {
        ...
        if(!result) return
        if (result.code === 0){
            let msg = `【常规脚本】途虎日报，结果：${JSON.stringify(result)}\n`
            console.log(msg)
        }else {
            let msg = `【常规脚本】途虎日报，错误数据：${JSON.stringify(result)}\n`
            notifyStr += msg
            console.log(msg)
        }
    } catch (e) {
...
await httpRequest('get', urlObject)
 let result = httpResult;
        if(!result) return
          if (result.code === 0){
            let msg = `【常规脚本】工作日查询，结果：${JSON.stringify(result)}\n`;
            daytype = result.type.type;
            console.log(msg)
        }else {
            let msg = `【常规脚本】工作日查询，错误数据：${JSON.stringify(result)}\n`
